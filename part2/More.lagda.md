...
rename ρ (`tt)          = `tt
rename ρ (`[])          = `[]
rename ρ (v `∷ w)       = (rename ρ v) `∷ (rename ρ w)
rename ρ (case[] x y z) = case[] (rename ρ x) (rename ρ y) (rename (ext (ext ρ)) z)
```
...
subst σ (`tt)          =  `tt
subst σ (`[])          = `[]
subst σ (v `∷ w)       = (subst σ v) `∷ (subst σ w)
subst σ (case[] x y z) = case[] (subst σ x) (subst σ y) (subst (exts (exts σ)) z)
```
...
data Value : ∀ {Γ A} → Γ ⊢ A → Set where
  ...
  V-∷  : ∀ {Γ A} {V : Γ ⊢ A} {W : Γ ⊢ List A} → Value V → Value W → Value (V `∷ W)
...
data _—→_ : ∀ {Γ A} → (Γ ⊢ A) → (Γ ⊢ A) → Set where
  ...
  ξ-case[] : ∀ {Γ A B} {L L' : Γ ⊢ List A} {M : Γ ⊢ B} {N : Γ , A , List A ⊢ B}
           → L —→ L'
             ----------------
           → case[] L M N —→ case[] L' M N
...
V¬—→ (V-inj₁ VL) (ξ-inj₁ L—→L')     = V¬—→ VL L—→L'
V¬—→ (V-inj₂ VL) (ξ-inj₂ L—→L')     = V¬—→ VL L—→L'
V¬—→ (V-∷ v w) (ξ-∷₁ step)          = V¬—→ v step
V¬—→ (V-∷ v w) (ξ-∷₂ x step)        = V¬—→ w step
```
...
... | step x—→x'                            = step (ξ-case[] x—→x')
... | done V-[]                             = step β-[]
... | done (V-∷ vx vx₁)                     = step (β-∷ vx vx₁)
```
...
zeroPlusSwapped : ∅ ⊢ (`ℕ ⇒ `ℕ) `⊎ `ℕ
zeroPlusSwapped = swap+ · zeroPlus
```
