...
1. `` (ƛ "y" ⇒ ` "x" · (ƛ "x" ⇒ ` "x")) ``
2. `` (ƛ "y" ⇒ ` "x" · (ƛ "x" ⇒ `zero)) ``
3. `` (ƛ "y" ⇒ `zero · (ƛ "x" ⇒ ` "x")) `` ← Correct answer
4. `` (ƛ "y" ⇒ `zero · (ƛ "x" ⇒ `zero)) ``
...
_[_:=_]' : Term → Id → Term → Term
substHelper : Id → Id → Term → Term → Term

substHelper x y V N with x ≟ y
... | yes _ = N
... | no  _ = N [ y := V ]'
...
... | yes _ = V
... | no  _ = ` x
(ƛ x ⇒ t) [ y := V ]' = ƛ x ⇒ (substHelper x y V t)
(t · t₁) [ y := V ]' = t [ y := V ]' · t₁ [ y := V ]'
`zero [ y := V ]' = `zero
(`suc t) [ y := V ]' = `suc (t [ y := V ]')  
case t [zero⇒ t₁ |suc x ⇒ t₂ ] [ y := V ]' = case (t [ y := V ]') [zero⇒ (t₁ [ y := V ]') |suc x ⇒ (substHelper x y V t₂) ]
(μ x ⇒ t) [ y := V ]' = μ x ⇒ substHelper x y V t

-- Examples that this behaves as intended:
_ : (ƛ "z" ⇒ ` "s" · (` "s" · ` "z")) [ "s" := sucᶜ ]'
      ≡ ƛ "z" ⇒ sucᶜ · (sucᶜ · ` "z")
_ = refl
...
1.  `` sucᶜ · (sucᶜ · `zero) ``
2.  `` (ƛ "z" ⇒ sucᶜ · (sucᶜ · ` "z")) · `zero `` ← Correct answer
3.  `` `zero ``
...
```agda
—↠-trans : ∀ {S T M} → S —↠ T → T —↠ M → S —↠ M
—↠-trans (_ ∎) T↠M = T↠M
—↠-trans {S} {T} {M} (_ —→⟨ x ⟩ S↠T) T↠M = S —→⟨ x ⟩ —↠-trans S↠T T↠M
...
—↠≲—↠′ = record { to = to
            ...
            where
              to : ∀ {S T} → (S —↠ T) → (S —↠′ T)
              to (_ ∎) = refl′
              to (_ —→⟨ x ⟩ s↠t) = trans′ (step′ x) (to s↠t)

              from : ∀ {S T} → (S —↠′ T) → (S —↠ T)
              from {S} {T} (step′ x) = S —→⟨ x ⟩ T ∎
              from {S} refl′ = S ∎
              from (trans′ s↠′t s↠′t₁) = —↠-trans (from s↠′t) (from s↠′t₁)

              from∘to : ∀ {S T} → (s↠t : S —↠ T) → from (to s↠t) ≡ s↠t
              from∘to (_ ∎) = refl
              from∘to (_ —→⟨ x ⟩ s↠t) rewrite from∘to s↠t = refl

              --to∘from : ∀ {S T} → (s↠t : S —↠′ T) → to (from s↠t) ≡ s↠t
              --to∘from (step′ x) = {!!} ((step′ x) ≡ trans′ (step′ x) refl′)) does not work
              --to∘from refl′ = refl
              --to∘from (trans′ x x₁) = {!!}
              -- It is not isomorphic because similar to the bin exercise without canonical
              -- binary numbers, —↠′ has several representations of the same proof. For example
              -- the step proof can be (step′ x) but also trans′ (step′ x) refl′
              --  We would need a canonical representations to form an isomorphism similar to the bin exercise
```
...
value-nf : ∀ {M N} → Value M → ¬ M —→ N
value-nf (V-suc vm) (ξ-suc m→n) = value-nf vm m→n

deterministic (ξ-·₁ L→L') (ξ-·₁ L→L'') rewrite deterministic L→L' L→L'' = refl
deterministic (ξ-·₁ L→L') (ξ-·₂ vl M→M') = ⊥-elim (value-nf vl L→L')
deterministic (ξ-·₂ vm M'→M') (ξ-·₁ V→L') = ⊥-elim (value-nf vm V→L')
deterministic (ξ-·₂ v M→M') (ξ-·₂ v' M→M'') rewrite deterministic M→M' M→M'' = refl
deterministic (ξ-·₂ v M→M') (β-ƛ vm) = ⊥-elim (value-nf vm M→M')
deterministic (β-ƛ v) (ξ-·₂ x₁ V→M') = ⊥-elim (value-nf v V→M')
deterministic (β-ƛ x) (β-ƛ x₁) = refl
deterministic (ξ-suc M→M') (ξ-suc M→M'') rewrite deterministic M→M' M→M'' = refl
deterministic (ξ-case L→L') (ξ-case L→L'') rewrite deterministic L→L' L→L'' = refl
deterministic (ξ-case (ξ-suc M→M')) (β-suc v) = ⊥-elim (value-nf v M→M')
deterministic β-zero β-zero = refl
deterministic (β-suc v) (ξ-case (ξ-suc V→M')) = ⊥-elim (value-nf v V→M')
...
diamond {L} {M} {N} (fst Data.Product., snd) rewrite deterministic fst snd = N Data.Product., (N ∎) Data.Product., (N ∎)
...
confluence {L} (_ ∎) (_ ∎) = L Data.Product., ((L ∎) Data.Product., (L ∎))
confluence {L} {M} {N} (_ ∎) (_ —→⟨ x ⟩ LN) = N Data.Product., ((L —→⟨ x ⟩ LN) Data.Product., (N ∎))
confluence {L} {M} (_ —→⟨ x ⟩ LM) (_ ∎) = M Data.Product., ((M ∎) Data.Product., (L —→⟨ x ⟩ LM))
confluence (_ —→⟨ x ⟩ LM) (_ —→⟨ x₁ ⟩ LN) rewrite deterministic x x₁ with confluence LN LM
... | p Data.Product., (p' Data.Product., p'') = p Data.Product., (p'' Data.Product., p')
```
...
_ =
  begin
    (μ "+" ⇒ ƛ "m" ⇒ ƛ "n" ⇒ case ` "m" [zero⇒ ` "n" |suc "m" ⇒ `suc (` "+" · ` "m" · ` "n") ]) · one · one
  —→⟨ ξ-·₁ (ξ-·₁ β-μ) ⟩
...
Γ≃List = record { to = to
            ...
            where
              to : ∀ (Γ : Context) → List (Id × Type)
              to ∅ = []
              to (Γ , x ⦂ T) = (x Data.Product., T) ∷ to Γ

              from : ∀ (lst : List (Id × Type)) → Context
              from [] = ∅
              from ((id Data.Product., typ) ∷ lst) = (from lst) , id ⦂ typ

              from∘to : ∀ (Γ : Context) → from (to Γ) ≡ Γ
              from∘to ∅ = refl
              from∘to (Γ , x ⦂ x₁) rewrite from∘to Γ = refl

              to∘from : ∀ (lst : List (Id × Type)) → to (from lst) ≡ lst
              to∘from [] = refl
              to∘from ((id Data.Product., typ) ∷ lst) rewrite to∘from lst = refl
```
...
2. `` ∅ , "y" ⦂ `ℕ ⇒ `ℕ , "x" ⦂ `ℕ ⊢ ` "x" · ` "y" ⦂ A `` ← Not derivable, x would need to be a function type which it is not
3. `` ∅ , "y" ⦂ `ℕ ⇒ `ℕ ⊢ ƛ "x" ⇒ ` "y" · ` "x" ⦂ A ``   ←   `ℕ ⇒ `ℕ

For each of the following, give types `A`, `B`, and `C` for which it is derivable,
or explain why there are no such types.

1. `` ∅ , "x" ⦂ A ⊢ ` "x" · ` "x" ⦂ B `` ← Not derivable. The only applicable rule here would be
the application rule. This would require x to be of type A ⇒ B and A at the same time, for some type A.
This is nothing we can derive from the given context and thus we will not be able to conclude a type.
In fact, the following counterproof shows this:
```agda
id-app : ∀ {A B} → ¬ (∅ , "x" ⦂ A ⊢ ` "x" · ` "x" ⦂ B)
id-app (⊢` (S x ()) · ⊢` Z)
```

2. `` ∅ , "x" ⦂ A , "y" ⦂ B ⊢ ƛ "z" ⇒ ` "x" · (` "y" · ` "z") ⦂ C `` ← A ≡ B ≡ C ≡ `ℕ ⇒ `ℕ 
