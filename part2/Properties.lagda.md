...
open import System-I.part2.Lambda
```
...
```
can-iso : ∀ {V A} → Canonical V ⦂ A ≃ (∅ ⊢ V ⦂ A) × (Value V)
can-iso = record { to = to
                 ; from = from
                 ; from∘to = from∘to
                 ; to∘from = to∘from
                 }
             where
               ...
               ... | ⟨ fst , snd ⟩ = ⟨ (⊢suc fst) , (V-suc snd) ⟩
        
               from : ∀ {V A} → (∅ ⊢ V ⦂ A) × (Value V) → Canonical V ⦂ A
               from ⟨ ⊢ƛ fst , snd ⟩ = C-ƛ fst
               from ⟨ ⊢zero , snd ⟩ = C-zero
               from ⟨ ⊢suc fst , V-suc snd ⟩ = C-suc (from ⟨ fst , snd ⟩)

               from∘to : ∀ {V A} → (cv : Canonical V ⦂ A) → from (to cv) ≡ cv
               from∘to (C-ƛ x) = refl
               from∘to C-zero = refl
               from∘to (C-suc cv) rewrite from∘to cv = refl

               to∘from : ∀ {V A} → (tv : (∅ ⊢ V ⦂ A) × (Value V)) → to (from tv) ≡ tv
               to∘from ⟨ ⊢ƛ fst , V-ƛ ⟩ = refl
               to∘from ⟨ ⊢zero , V-zero ⟩ = refl
               to∘from ⟨ ⊢suc fst , V-suc snd ⟩ rewrite to∘from ⟨ fst , snd ⟩ = refl
```
...
Progress-≃ = record { to = to
                ...
                where
                  ...
                  to∘from : ∀ {M} → (em : (Value M ⊎ ∃[ N ](M —→ N))) → to (from em) ≡ em
                  to∘from (inj₁ x) = refl
                  to∘from (inj₂ y) = refl
```
...
progress′ (T · T') (∅⊢T⦂A · ∅⊢T'⦂A₁) with progress′ T ∅⊢T⦂A
... | inj₂ ⟨ N , st ⟩ = inj₂ ⟨ (N · T') , (ξ-·₁ st) ⟩
... | inj₁ vt with progress′ T' ∅⊢T'⦂A₁
... | inj₂ ⟨ N , st' ⟩ = inj₂ ⟨ (T · N) , (ξ-·₂ vt st') ⟩
progress′ ((ƛ x ⇒ T) · T') (∅⊢T⦂A · ∅⊢T'⦂A₁) | inj₁ vt | inj₁ vt' = inj₂ ⟨ T [ x := T' ] , β-ƛ vt' ⟩
progress′ .`zero ⊢zero = inj₁ V-zero
progress′ (`suc L) (⊢suc ∅⊢M⦂A) with progress′ L ∅⊢M⦂A
... | inj₁ vm = inj₁ (V-suc vm)
... | inj₂ ⟨ N , sm ⟩ = inj₂ ⟨ `suc N , (ξ-suc sm) ⟩
progress′ (case L [zero⇒ L' |suc x ⇒ L'' ]) (⊢case ∅⊢L⦂A ∅⊢M⦂A₁ ∅⊢M⦂A₂) with progress′ L ∅⊢L⦂A
... | inj₁ V-zero = inj₂ ⟨ L' , β-zero ⟩
progress′ case .(`suc V) [zero⇒ L' |suc x ⇒ L'' ] (⊢case ∅⊢L⦂A ∅⊢M⦂A₁ ∅⊢M⦂A₂) | inj₁ (V-suc {V} vl) = inj₂ ⟨ L'' [ x := V ] , β-suc vl ⟩
... | inj₂ ⟨ N , sl ⟩ = inj₂ ⟨ case N [zero⇒ L' |suc x ⇒ L'' ] , (ξ-case sl) ⟩
...
value? (⊢ƛ ∅⊢M⦂A) = yes V-ƛ
value? (∅⊢L⦂A · ∅⊢M⦂A₁) with progress ∅⊢L⦂A
... | step L→L' = no (λ vlm → —→¬V (ξ-·₁ L→L') vlm)
... | done vl with progress ∅⊢M⦂A₁
... | step M→M' = no (λ vlm → —→¬V (ξ-·₂ vl M→M') vlm)
value? (∅⊢L⦂A · ∅⊢M⦂A₁) | done V-ƛ | done vm = no (λ vlm → —→¬V (β-ƛ vm) vlm)
value? ⊢zero = yes V-zero
value? (⊢suc ∅⊢M⦂A) with progress ∅⊢M⦂A
... | step M→M' = no (λ vsuc → —→¬V (ξ-suc M→M') vsuc)
... | done v = yes (V-suc v)
value? (⊢case ∅⊢L⦂A ∅⊢M⦂A₁ ∅⊢N⦂A₂) with progress ∅⊢L⦂A
... | step L→L' = no (λ vcase → —→¬V (ξ-case L→L') vcase)
... | done V-zero = no (λ vcase → —→¬V β-zero vcase)
... | done (V-suc v) = no (λ vcase → —→¬V (β-suc v) vcase)
value? (⊢μ ∅⊢M⦂A) = no (λ vμ → —→¬V β-μ vμ)
```
...
substHelper' : ∀ {Γ x y N V A B C}
             → Γ , x ⦂ A , y ⦂ B ⊢ N ⦂ C
             → ∅ ⊢ V ⦂ A
             → Γ , y ⦂ B ⊢ substHelper y x V N  ⦂ C



substHelper' {x = x} {y} ⊢N ⊢V with y ≟ x
... | yes refl = drop ⊢N
... | no  y≢x = subst' ⊢V (swap y≢x ⊢N)
...
... | no  x≢y = ⊢` ∋x
subst' {x = y} ⊢V (⊢ƛ {x = x} ⊢N) = ⊢ƛ (substHelper' ⊢N ⊢V) 
subst' ⊢V (⊢L · ⊢M) = (subst' ⊢V ⊢L) · (subst' ⊢V ⊢M)
subst' ⊢V ⊢zero = ⊢zero
subst' ⊢V (⊢suc ⊢N) = ⊢suc (subst' ⊢V ⊢N)
subst' {x = y} ⊢V (⊢case {x = x} ⊢N ⊢N₁ ⊢N₂) = ⊢case (subst' ⊢V ⊢N) (subst' ⊢V ⊢N₁) (substHelper' ⊢N₂ ⊢V)
subst' {x = y} ⊢V (⊢μ {x = x} ⊢N) = ⊢μ (substHelper' ⊢N ⊢V)
```
...
```agda
-- I will use one so it is not so ridiculously long
⊢1*1 : ∅ ⊢ (mul · one) · one ⦂ `ℕ
⊢1*1 = (⊢mul · (⊢suc ⊢zero)) · (⊢suc ⊢zero)
...
```agda
-- We know that zero is a well typed value
_ : ∅ ⊢ `zero ⦂ `ℕ
_ = ⊢zero
-- Looking at the following term
counter₁ : Term
counter₁ = case `zero [zero⇒ `zero |suc "x" ⇒ ƛ "z" ⇒ `"z" ]
-- we notice that it can not be well-typed:
counter₁-proof : ∀ {A} → ¬ ∅ ⊢ counter₁ ⦂ A
counter₁-proof = λ{ (⊢case ⊢zero ⊢zero ())}
-- However, it does step to zero:
counter₁-step : counter₁ —→ `zero
counter₁-step = β-zero

-- For an example without case consider the following term
counter₂ : Term
counter₂ = (ƛ "y" ⇒ `zero) · (ƛ "z" ⇒ `zero · `zero)
-- Again, this one can not be well typed:
counter₂-proof : ∀ {A} → ¬ ∅ ⊢ counter₂ ⦂ A
counter₂-proof = λ{ (x · ⊢ƛ (() · x₂))}
-- However, it does step to zero:
counter₂-step : counter₂ —→ `zero
counter₂-step = β-ƛ V-ƛ
```
...
... | step M→M' = λ{ ⟨ ¬M→M' , ¬vm ⟩ → ¬M→M' M→M'}
... | done vm   = λ{ ⟨ ¬M→M' , ¬vm ⟩ → ¬vm vm}
...
preserves ⊢M (_ ∎) = ⊢M
preserves ⊢M (_ —→⟨ x ⟩ M↠N) = preserves (preserve ⊢M x) M↠N
...
false, give a counterexample:
  ...
  - Determinism [FAILS] (Easy to see)

  - Progress [FAILS] zap itself is well-typed but neither a value nor can it take a step

  - Preservation [FAILS] zap itself can have any type, hence we can step from a type B to A
...
false, give a counterexample:
  ...
  - Preservation [FAILS] (foo itself is not typed but the identity function is)
